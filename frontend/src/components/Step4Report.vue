            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section content generated (the full section may still be in progress) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After the DOM updates, adjust scroll position so the button stays in the same spot
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract the forecast scenario
    const reqMatch = text.match(/(?:Forecast Scenario|预测场景):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract stats - match the "related forecast facts: X items" format
    ...
    // Extract sub-questions in full, with no count limit
    const subQSection = text.match(/### (?:Analysis Sub-questions|分析的子问题|Sub-questions)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }
    
    // Extract key facts in full, with no count limit
    ...
    // Extract core entities in full, including summary and related fact count
    ...
    // Extract relationship chains in full, with no count limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current valid facts in full, with no count limit
    ...
    // Extract historical/expired facts in full, with no count limit
    ...
    // Extract involved entities in full, with no count limit
    const entitySection = text.match(/### (?:\[Entities Involved\]|\[Involved Entities\]|【涉及实体】)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const topicMatch = text.match(/(?:\*\*Interview Topic:\*\*|Interview Topic|\*\*采访主题:\*\*|采访主题):\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // Extract the interview count (for example, "5 / 9 simulated agents")
    const countMatch = text.match(/(?:\*\*Interview Count:\*\*|Interview Count|\*\*采访人数:\*\*|采访人数):\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // Extract the rationale for selecting interviewees
    const reasonMatch = text.match(/### (?:Interviewee Selection Rationale|采访对象选择理由)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcripts|Interview Transcript|采访实录))/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: - **parent_601 (index 0)**: as a representative parent...
        if (!headerMatch) {
        ...
        if (name) {
          // Save the previous person's rationale
          ...
          // Continuation of the rationale, excluding the closing summary paragraph
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract the title (for example, "student" or "education practitioner")
      ...
      if (bioMatch) {
        interview.bio = (bioMatch[1] || bioMatch[2] || bioMatch[3] || bioMatch[4] || '').trim().replace(/\.\.\.$/, '...')
      }
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=(?:\*\*Key Quotes|Key Quotes:|\*\*关键引言:\*\*|关键引言:)|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/(?:\[Twitter Answer\]|\[Twitter回答\])\n?([\s\S]*?)(?=(?:\[Reddit Answer\]|\[Reddit回答\])|$)/)
        const redditMatch = answerText.match(/(?:\[Reddit Answer\]|\[Reddit回答\])\n?([\s\S]*?)$/)
        ...
        // Platform fallback logic for older formats with only one platform marker
        if (!twitterMatch && redditMatch) {
          // Only a Reddit answer exists; copy it as the default display only when it is not placeholder text
          if (interview.redditAnswer && !['(No reply from this platform)', '（该平台无回复）'].includes(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && !['(No reply from this platform)', '（该平台无回复）'].includes(interview.twitterAnswer)) {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No per-platform markers exist (very old format); treat the whole block as the answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes, compatible with multiple quote styles
      const quotesMatch = block.match(/(?:\*\*Key Quotes:\*\*|Key Quotes:|\*\*关键引言:\*\*|关键引言:)\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // Prefer the > "text" format
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/(?:Found\s*(\d+)\s*items|找到\s*(\d+)\s*条相关信息)/i)
    if (countMatch) result.count = parseInt(countMatch[1] || countMatch[2])
    
    // Extract related facts in full, with no count limit
    ...
    // Try to extract edge data, if present
    ...
    // Try to extract node data, if present
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Analysis Sub-questions Generated from Drift Queries'),
            ...
            (expandedHistorical.value ? props.result.historicalFacts : props.result.historicalFacts.slice(0, INITIAL_SHOW_COUNT)).map((fact, i) => 
              h('div', { class: 'fact-item historical', key: i }, [
                ...
                h('div', { class: 'fact-content' }, [
                  // Try extracting time information in [time - time] format
                  (() => {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether this is platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return (
        t === '(No reply from this platform)'
        // Legacy CN placeholders (escaped to avoid Chinese characters in source)
        || t === '\uFF08\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D\uFF09'
        || t === '(\u8BE5\u5E73\u53F0\u672A\u83B7\u5F97\u56DE\u590D)'
        || t === '[\u65E0\u56DE\u590D]'
      )
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (preferred)
      // 2. Legacy CN: "\u95EE\u9898X:" (escaped)
      // 3. "1. " or "\n1. " (legacy numbered format)
      let matches = []
      let match

      // First try the "Question X:" format
      ...
      // Then try legacy CN "\u95EE\u9898X:" format (escaped)
      if (matches.length === 0) {
        const legacyCnPattern = /(?:^|[\r\n]+)\u95EE\u9898(\d+)[:：]\s*/g
        while ((match = legacyCnPattern.exec(answerText)) !== null) {
          matches.push({
            num: parseInt(match[1]),
            index: match.index,
            fullMatch: match[0]
      ...
      // If nothing matches, fall back to the "number." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbering is found, or only one number is found, return the whole answer
      if (matches.length <= 1) {
      ...
      // Extract each segment by number
      const parts = []
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split succeeded and the index is valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Split failed: return the full answer for the first question and empty strings for the rest
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has answers from both platforms, excluding placeholder text
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers, and the content differs
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch, shown only when both real platform replies exist
...
// Current active step, used for the top header display
const activeStep = computed(() => {
  ...
  // If there is no active step, return the last completed step
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading level-2 heading (## xxx) because the section title is already shown outside
  ...
  // Clean cases where <p><br> appears directly before a block element due to extra blank lines
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> tags before paragraph openings that follow block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists by preserving increments when single-item <ol> blocks are split by paragraphs
...
const getLogLevelClass = (log) => {
  ...
  // INFO uses the default color and is not marked as success
  return ''
}
      ...
      if (newLogs.length > 0) {
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            isComplete.value = true
            currentSectionIndex.value = null  // Ensure the loading state is cleared
            emit('update-status', 'completed')
            stopPolling()
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is complete, scroll to the top; otherwise scroll to the bottom to follow the latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try extracting content inside the <final_answer> tag
  ...
  // Try extracting content after "Final Answer:" in multiple supported formats
  ...
  // Legacy CN marker (escaped)
  const legacyCnFinalMatch = response.match(/\u6700\u7EC8\u7B54\u6848[:：]\s*\n*([\s\S]*)$/i)
  if (legacyCnFinalMatch) {
    return legacyCnFinalMatch[1].trim()
  }
  
  // If it starts with ##, #, or >, it may already be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If the content is long and includes markdown, try removing the reasoning preamble
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the reasoning block that starts with "Thought:"
...
/* Console Logs - keep aligned with Step3Simulation.vue */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
