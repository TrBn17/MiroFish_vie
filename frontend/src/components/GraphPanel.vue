...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Show the end hint when switching from simulating to idle
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle self-loop expansion state
...
const renderGraph = () => {
  ...
  // Process edge data and calculate counts/indexes for each node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store details for all self-loop edges
        }
      })
      return
    }
    ...
    // Calculate curvature: spread multiple edges apart, keep a single edge straight
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly so edges remain distinguishable
      // Increase curvature range as edge count grows
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if the edge direction is opposite to the normalized direction
      // This keeps all edges distributed in the same frame of reference and avoids overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation: adjust node spacing dynamically based on edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust distance dynamically based on the number of edges between this node pair
      // Base distance is 150; add 40 for each extra edge
    ...
    // Add centering force so disconnected node groups gather toward the middle
  ...
  // Links: use paths to support curves
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the right side of the node and loop back around
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Draw the self-loop with an arc (sweep-flag=1, clockwise)
    ...
    // Calculate curve control points and adjust them dynamically by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the link direction, scaled by distance to keep curves visible
    // The more edges there are, the larger the offset ratio becomes
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, add 5% for each extra edge
  ...
  // Calculate curve midpoints for label positioning
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier formula B(t) = (1-t)^2 P0 + 2(1-t)tP1 + t^2 P2, t=0.5
  ...
  // Link label backgrounds (white backgrounds improve readability)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position and do not restart the simulation yet (distinguish click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect when a real drag starts (movement exceeds the threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only after a real drag is first detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Let the simulation cool down only if an actual drag happened
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation for clearer horizontal labels)
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Remove rotation and keep labels horizontal
    })
  ...
  // Close the detail panel when clicking empty space
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
