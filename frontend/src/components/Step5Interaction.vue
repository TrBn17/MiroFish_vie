          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">A fast conversational version of the report generation agent. It can call four specialized tools and has full access to MiroFish memory.</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Aligns real-world seed data with simulation state and combines global and local memory to provide deep cross-temporal attribution analysis.</div>
                  </div>
                </div>
                ...
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">A GraphRAG-based instant query interface optimized for index efficiency, designed to quickly extract specific node attributes and discrete facts.</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">Runs autonomous interviews and can hold parallel multi-turn conversations with simulated individuals to collect unstructured opinions and psychological state data.</div>
                  </div>
                </div>
              </div>
            </div>
...
// Save the current chat history to the cache
...
const renderMarkdown = (content) => {
  ...
  // Clean <p><br> before block elements when extra blank lines create them
  ...
  // Fix numbering for non-contiguous ordered lists so numbering continues when single-item <ol> blocks are separated by paragraphs
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Automatically save chat history to the cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Format: {"twitter_0": {...}, "reddit_0": {...}} or a single-platform object like {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // Convert the object map to an array-like flow and prefer the reddit reply first
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer the reddit reply, then fall back to twitter
...
const submitSurvey = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Convert the object map into an array-like result list
...
/* Fix ordered-list numbering with CSS counters so multiple ol blocks continue numbering */
.message-text {
...
/* Quote styles for the chat/survey area */
