<template>
  ...
  >
    <!-- Background decoration: tech grid (shown only when there are projects) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (first ~20 chars of the requirement) -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">Requirement</div>
            ...
            <!-- Playback limitation hint -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 “Start Simulation” and Step 5 “Deep Interaction” must be started while running and do not support playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Expanded state: compute height dynamically based on the card count
  ...
  // Calculate the required height: rows * card height + (rows - 1) * gap + a small bottom buffer
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate the number of cards in the current row so each row stays centered
    ...
    // Expand downward and increase the spacing from the title
    ...
    const x = offset * 35
    // Adjust the starting position so it stays near the title with proper spacing
...
// Get the style class from round progress
...
// Generate a title from the simulation requirement (first ~20 characters)
...
// Format the simulation_id display (first 6 characters)
...
// Get the file type (used for styling)
...
// Truncate filenames while keeping the extension
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state, even if animation is still in progress
        pendingState = shouldExpand
        
        // Clear the previous debounce timer because new scroll intent should override the old one
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If animation is in progress, only record the state and handle it after the animation ends
        if (isAnimating) return
        
        // If the target state matches the current state, there is nothing to do
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Delay state switches with debouncing to avoid rapid flicker
        // Use a shorter delay for expand (50ms) and a longer one for collapse (200ms) for better stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending state still needs to run because later scrolling may have overridden it
          ...
          // Clear the lock after the animation ends and check for pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After the animation ends, check for a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay execution briefly to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin so the viewport bottom contracts upward and requires more scrolling before expansion
...
// Watch route changes and reload data when returning to the home page
...
onMounted(async () => {
  // Make sure the DOM has rendered before loading data
  await nextTick()
  await loadHistory()
...
/* Use a CSS background pattern to create a square grid with fixed spacing */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor from the top left so height changes extend only downward and do not shift the existing grid */
...
.cards-container {
  ...
  /* min-height is computed dynamically in JS based on the card count */
}
...
/* Low-saturation palette - Morandi colors */
