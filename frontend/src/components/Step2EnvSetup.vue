    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using the full context, the system automatically calls tools to organize entities and relations from the knowledge graph, initialize simulated individuals, and give them unique behavior and memories based on real-world seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Topics Currently Linked to Real-World Seeds</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the simulation requirements and real-world seeds, the LLM intelligently configures world time flow, recommendation algorithms, each individual's active hours, posting frequency, event triggers, and more.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, the system automatically generates initial activation events and hot topics to guide the simulation world's starting state.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">The simulation environment is ready. You can start running the simulation now.</p>
          
          <!-- Round config: shown only after config generation completes and the round count is available -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Settings</span>
                <span class="section-desc">MiroFish automatically plans and simulates <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time, and each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time passing</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If the agent count is 100: estimated runtime about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, we strongly recommend switching to Custom mode to reduce the number of rounds so you can preview results quickly and lower the risk of errors -></p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Apparent Age in Event</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years old</span>
            </div>
            <div class="info-item">
              <span class="info-label">Apparent Gender in Event</span>
            ...
            <div class="info-item">
              <span class="info-label">Apparent MBTI in Event</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Summary of experience and preferred style of action</span>
              </div>
              <div class="dimension-card">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">The individual's link and interaction graph</span>
              </div>
            </div>
...
const props = defineProps({
  simulationId: String,  // Passed in from the parent component
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// Deduplicate logs by tracking the last key message
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation round settings
const useCustomRounds = ref(false) // Use automatically generated rounds by default
const customMaxRounds = ref(40)   // Recommend 40 rounds by default
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generate Agent Personas' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter the config generation stage and start polling for config
    if (!configTimer) {
...
// Calculate the auto-generated round count from the config without a hardcoded default
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null when the config is not generated yet
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Return null when the config data is incomplete
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Ensure the max round count is at least 40 (the recommended value) so the slider range stays valid
...
// Get the matching username by agent_id
...
// Handle clicks on the start simulation button
const handleStartSimulation = () => {
  // Build the params passed to the parent component
  const params = {}
  
  if (useCustomRounds.value) {
    // If the user customizes the rounds, pass max_rounds
    params.maxRounds = customMaxRounds.value
    addLog(`Start simulation with custom rounds: ${customMaxRounds.value} rounds`)
  } else {
    // If the user keeps the auto-generated rounds, do not pass max_rounds
    addLog(`Start simulation with auto-configured rounds: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // Mark the first step as complete and start step two
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Detected an existing completed setup. Reusing it directly.')
      ...
      // Set the expected total agent count immediately from the prepare API response
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Output detailed progress logs while avoiding duplicates
        ...
        // Extract stage info from the message
        const match = data.message.match(/\[(\d+)\/(\d+)\]\s*([^:]+)/)
        if (match) {
          currentStage.value = match[3].trim()
        }
        // Output message logs while avoiding duplicates
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output persona generation progress logs only when the count changes
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output config generation stage logs while avoiding duplicates
      ...
      // If the config has already been generated
      if (data.config_generated && data.config) {
        simulationConfig.value = data.config
        addLog('✓ Simulation config generated')
        
        // Show a detailed config summary
        if (data.summary) {
        ...
        // Show time setting details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(`Time settings: ${tc.minutes_per_round} minutes per round, total ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup complete. You can start the simulation now.')
...
const loadPreparedData = async () => {
  ...
  // Get the config using the realtime endpoint
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // The config is not generated yet. Start polling.
        addLog('Config still generating. Start polling and wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
/* Simulation round setting styles */
