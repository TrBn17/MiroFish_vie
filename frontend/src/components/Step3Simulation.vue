...
// Computed
// Display actions in chronological order (newest at the end, at the bottom)
const chronologicalActions = computed(() => {
  return allActions.value
})

// Action counts by platform
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any polling that may still be running
}
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so the previous run does not affect this one
  resetAllState()
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // Force a fresh restart
...
// Track the previous round for each platform to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes for each platform separately and log them
      ...
      // Extra check: the backend may not have updated runner_status yet even though the platforms already report completion
      // Check via twitter_completed and reddit_completed
...
const checkPlatformsCompleted = (data) => {
  ...
  // Use actions_count to infer whether a platform was enabled (if count > 0 or running was ever true)
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  const redditEnabled = (data.reddit_actions_count > 0) || data.reddit_running || redditCompleted
  
  // Return false if no platform was enabled
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll so users can inspect the timeline freely
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('The report generation request has already been sent. Please wait...')
    return
  }
