...
const handleNextStep = (params = {}) => {
  ...
  // Record max-rounds settings
  if (params.maxRounds) {
    addLog(`Custom max rounds: ${params.maxRounds} rounds`)
  } else {
    addLog('Using auto-configured max rounds')
  }
  ...
  // If maxRounds is customized, pass it via query params
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Detected a running simulation environment, shutting down...')
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful close fails, try a forced stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Error while closing simulation environment: ${closeErr.message}`)
        // If graceful close throws, try a forced stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failing to check environment status should not block the rest of the flow
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Error while force-stopping simulation: ${err.message}`)
  }
}
