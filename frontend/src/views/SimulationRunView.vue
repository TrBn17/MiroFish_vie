...
// Read maxRounds from the query during initialization so child components can access it immediately
...
const handleGoBack = async () => {
  ...
  try {
    // Try a graceful simulation environment shutdown first
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // The environment is not running, check whether the process still needs to be stopped
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Get the simulation config to read minutes_per_round
      try {
        const configRes = await getSimulationConfig(currentSimulationId.value)
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          minutesPerRound.value = configRes.data.time_config.minutes_per_round
          addLog(`Time settings: ${minutesPerRound.value} minutes per round`)
        }
      } catch (configErr) {
        addLog(`Failed to get time settings, using default: ${minutesPerRound.value} minutes/round`)
      }
...
const loadGraph = async (graphId) => {
  // While simulating, do not show the full-screen loading state during auto-refresh to avoid flicker
  // Show loading during manual refresh or the initial load
  if (!isSimulating.value) {
...
onMounted(() => {
  ...
  // Log the maxRounds setting (already read from the query during initialization)
  if (maxRounds.value) {
