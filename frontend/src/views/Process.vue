<template>
  <div class="process-page">
    ...
    <div class="main-content">
      ...
      <!-- Right side: build workflow details -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After documents are uploaded, the LLM analyzes their contents and automatically generates an ontology structure suitable for public opinion simulation (entity types + relationship types).
                </div>
              </div>
...
const currentPhase = ref(-1) // -1: uploading, 0: generating ontology, 1: graph build, 2: complete
...
// Handle a new project by calling the ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Start graph construction automatically
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Start graph data polling separately from task status polling
      startGraphPolling()
...
const fetchGraphData = async () => {
  try {
    // Fetch project information first to get graph_id
    ...
    if (projectResponse.success && projectResponse.data.graph_id) {
      ...
      if (graphResponse.success && graphResponse.data) {
        ...
        // Update the rendering only when data changes
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        if (projectResponse.success) {
          ...
          // Load the full graph data at the end
...
const renderGraph = () => {
  ...
  // Create a node map for name lookups
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // Create a node ID set for filtering valid edges
  ...
  // Draw edges, including wide transparent click targets
  ...
  simulation.on('tick', () => {
    // Update positions for all edges, including visible lines and transparent click targets
