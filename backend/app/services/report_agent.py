...
Sử dụng LangChain + Zep để tạo báo cáo mô phỏng theo mô hình ReACT

Chức năng:
1. Tạo báo cáo dựa trên yêu cầu mô phỏng và thông tin đồ thị Zep
2. Lập kế hoạch cấu trúc mục lục trước, sau đó tạo từng phần
3. Mỗi phần sử dụng mô hình suy nghĩ và tự phản tư nhiều vòng ReACT
4. Hỗ trợ hỏi đáp với người dùng và tự chủ gọi công cụ truy xuất trong hội thoại
"""
...
Đây là hàm truy xuất mạnh mẽ của chúng ta, được thiết kế cho phân tích chuyên sâu. Nó sẽ:
1. Tự động tách câu hỏi của bạn thành nhiều câu hỏi con
2. Truy xuất thông tin trong đồ thị mô phỏng theo nhiều chiều
3. Tổng hợp kết quả từ tìm kiếm ngữ nghĩa, phân tích thực thể và lần theo chuỗi quan hệ
4. Trả về nội dung truy xuất đầy đủ và sâu nhất

【Tình huống sử dụng】
- Cần phân tích sâu một chủ đề nào đó
- Cần hiểu nhiều mặt của một sự kiện
- Cần thu thập tư liệu phong phú để hỗ trợ cho từng chương báo cáo

【Nội dung trả về】
- Nguyên văn các sự kiện liên quan (có thể trích dẫn trực tiếp)
- Nhận định cốt lõi về thực thể
- Phân tích chuỗi quan hệ"""

TOOL_DESC_PANORAMA_SEARCH = """\
【Tìm kiếm toàn cảnh - Lấy góc nhìn tổng thể】
Công cụ này dùng để lấy bức tranh tổng thể đầy đủ của kết quả mô phỏng, đặc biệt phù hợp khi cần hiểu quá trình diễn biến của sự kiện. Nó sẽ:
1. Lấy tất cả các nút và mối quan hệ liên quan
2. Phân biệt sự kiện còn hiệu lực hiện tại với sự kiện lịch sử/hết hiệu lực
3. Giúp bạn hiểu dư luận đã thay đổi ra sao

【Tình huống sử dụng】
- Cần hiểu đầy đủ mạch phát triển của sự kiện
- Cần so sánh biến động dư luận ở các giai đoạn khác nhau
- Cần lấy thông tin toàn diện về thực thể và mối quan hệ

【Nội dung trả về】
- Sự kiện đang còn hiệu lực (kết quả mô phỏng mới nhất)
- Sự kiện lịch sử/hết hiệu lực (bản ghi diễn biến)
- Tất cả thực thể liên quan"""

TOOL_DESC_QUICK_SEARCH = """\
【Tìm kiếm đơn giản - Truy xuất nhanh】
Công cụ truy xuất nhanh gọn nhẹ, phù hợp cho các truy vấn thông tin đơn giản và trực tiếp.
...
【Nội dung trả về】
- Danh sách sự kiện liên quan nhất với truy vấn"""

TOOL_DESC_INTERVIEW_AGENTS = """\
【Phỏng vấn chuyên sâu - Phỏng vấn Agent thật sự (hai nền tảng)】
Gọi API phỏng vấn của môi trường mô phỏng OASIS để phỏng vấn thật sự các Agent đang chạy!
Đây không phải mô phỏng bằng LLM, mà là gọi giao diện phỏng vấn thật để lấy câu trả lời gốc của Agent mô phỏng.
Mặc định sẽ phỏng vấn đồng thời trên Twitter và Reddit để thu được góc nhìn đầy đủ hơn.

Quy trình hoạt động:
1. Tự động đọc tệp hồ sơ nhân vật để hiểu toàn bộ Agent mô phỏng
2. Chọn thông minh những Agent liên quan nhất với chủ đề phỏng vấn (như sinh viên, truyền thông, cơ quan chức năng...)
3. Tự động tạo câu hỏi phỏng vấn
4. Gọi API `/api/simulation/interview/batch` để phỏng vấn thật sự trên cả hai nền tảng
5. Tổng hợp mọi kết quả phỏng vấn để đưa ra phân tích đa góc nhìn

【Tình huống sử dụng】
- Cần hiểu quan điểm của sự kiện từ nhiều vai trò khác nhau (sinh viên nghĩ gì? truyền thông nhìn nhận ra sao? phía chính thức nói gì?)
- Cần thu thập ý kiến và lập trường từ nhiều bên
- Cần lấy câu trả lời thật của Agent mô phỏng (từ môi trường OASIS)
- Muốn báo cáo sinh động hơn, có kèm "bản ghi phỏng vấn"

【Nội dung trả về】
- Thông tin danh tính của Agent được phỏng vấn
- Câu trả lời phỏng vấn của từng Agent trên Twitter và Reddit
- Trích dẫn then chốt (có thể trích dẫn trực tiếp)
